...
if( jQuery.fn['dialog1'] ) {
	var createModal = function (aIDs, content, p, insertSelector, posSelector, appendsel) {
		var ms = $("<span id='"+aIDs.themodal+"'></span>"),
		mc = $("<div id='"+aIDs.modalcontent+"' class='ui-jqdialog-content' style='display:none;' title='"+p.caption+"'></div>").append(content);
		if(appendsel===true) {
			jQuery('body').append(mc);
			$(mc).attr("id",aIDs.themodal).css("display","").dialog({autoOpen: false });
			ms=null; 
	...
	var viewModal = function (selector,p) {
		if((p.left ==0 && p.top==0) || (!p.left || !p.top)) {
			var pos = [];
			pos = findPos($(selector)[0]) ;
			p.left = pos[0] + 4;
			p.top = pos[1] + 30;
		}
		if (p.width == 0 || !p.width) {p.width = 500;}
		...
		if (p.destroy===false) {
			...
			var dlg = $(selector+" div:first");
			prm = $.extend({},
				prm,
				{ close :function(e, ui){ $(this).dialog('destroy');$(dlg).appendTo(selector); } }
			);
			$(dlg).dialog(prm);
		}
	};
	var hideModal = function (selector, dlg) {
		if(dlg) {
			$(dlg).dialog('destroy').appendTo(selector);
		} else {
