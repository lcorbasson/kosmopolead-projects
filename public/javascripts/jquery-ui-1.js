...
function Datepicker() {
	...
	this._defaults = { // Global defaults for all the date picker instances
        ...
        showTime: false, // True to show Time Selection
        showTimeAm: false, // True to show Time Selection
        update:false
	};
...
$.extend(Datepicker.prototype, {
	...
	_newInst: function(target, inline) {
		...
		return {id: id, input: target, // associated target
            ...
            selectedHour: 0, selectedMinute: 0, // Time Selection
	...
	_optionDatepicker: function(target, name, value) {
		...
		if (inst) {
			...
			extendRemove(inst, {rangeStart: null, // start of range
                ...
                selectedHour: date.getHours(), selectedMinute:date.getMinutes(), // Time Selection
    ...
    /* Action for selecting a hour. */
    _selectHour: function(id,select){
        var target = $(id);
        var inst = this._getInst(target[0]);        
        inst.selectedHour=parseInt($(select).val());
		this._selectDate(id, this._formatDate(inst));
    },

	/* Action for selecting a minute. */
	_selectMinute: function(id,select) {
        var target = $(id);
        var inst = this._getInst(target[0]);
        inst.selectedMinute=select.value;
		this._selectDate(id, this._formatDate(inst));
	},

	/* Action for selecting a am or pm. */
	_selectAm: function(id,select) {
        var target = $(id);
        var inst = this._getInst(target[0]);
        inst.selectedAm=select.value;
        if (select.value=="am"){
            if (inst.selectedHour>12){
                inst.selectedHour=inst.selectedHour-12;
            }
        }else{
              if (inst.selectedHour<=12){
                inst.selectedHour=inst.selectedHour+12;
	...
	_selectDate: function(id, dateStr) {    
		...
		if (inst.inline) {
            if (this._get(inst, 'update')!=false)
                this._updateDatepicker(inst);
        }
        else{
            if (this._get(inst, 'showTime')==true){
                this._updateDatepicker(inst);
            }
            else{
                if (!inst.stayOpen) {
		...
		for (var iFormat = 0; iFormat < format.length; iFormat++) {
			if (literal)
				...
				switch (format.charAt(iFormat)) {
                    ...
                    case 'a':
//                        if  (date.getHours()>12){
//                            output +=   formatNumber('a', "pm", 2);
//                        }else{
//                            output +=   formatNumber('a', "am", 2);
//                        }
//                        break;
                    case 'A':
//                        if  (date.getHours()>12){
...
//                        break;
			...
			for (var iFormat = 0; iFormat < format.length; iFormat++) {
				if (literal)
					...
					switch (format.charAt(iFormat)) {
                        ...
                        case 'G':
                            // 24 H, without zero
                            output += formatNumber('G', date.getHours(), 1);
                            break;
                        case 'g':
                            if  (date.getHours()>12){
                                output +=   formatNumber('g', date.getHours()-12, 1);
                            }else{
                                output +=   formatNumber('g', date.getHours(), 1);
                            }
                            break;
                        ...
                        case 'h':
                            if  (date.getHours()>12){
                                output +=   formatNumber('g', date.getHours()-12, 2);
                            }else{
                                output +=   formatNumber('g', date.getHours(), 2);
                            }
                            break;
                        case 'i':
	...
	_setDateFromField: function(inst) {
		...
		inst.selectedHour = (dates ? date.getHours() : 0);
		inst.selectedMinute = (dates ? date.getMinutes() : 0);
	...
	/* Set the date(s) directly. */
	_setTime: function(inst, date) {
        inst.selectedHour = inst.currentHour = date.getHours();
        inst.selectedMinute = inst.currentMinute = date.getMinutes();
	},

	/* Set the date(s) directly. */
	_getTime: function(inst) {
        return  inst.currentHour + inst.currentMinute;
	},
	...
	_generateHTML: function(inst) {
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						tbody += '<td  class="' +
							...
							(unselectable ? '<span class="ui-state-default">' + printDate.getDate() + '</span>' : '<a id ="'+printDate+'" class="ui-state-default' +
        ...
        var showTimeAm = this._get(inst, 'showTimeAm');
        if (showTime==true){
            html += '<div class="ui-datepicker-buttonpane ui-widget-content">';
            html +='<div style="float:left;margin-top:5px"><select onchange="DP_jQuery.datepicker._selectHour(\'#' + inst.id + '\',this)">';
            if (showTimeAm==false){
                for (var h = 0; h < 24; h++) {
                    if (h==inst.selectedHour){
                        html +='<option selected>'+h+'</option>';
                    }else{
                ...
                if (inst.selectedHour>12){
                    hour_temp=inst.selectedHour-12;
                }else{
                    hour_temp=inst.selectedHour;
                }
                for (var h = 0; h < 13; h++) {
                    if (h==hour_temp){
                        html +='<option selected>'+h+'</option>';
            ...
            html +='</select>';
            html +='<select onchange="DP_jQuery.datepicker._selectMinute(\'#' + inst.id + '\',this)">';
            for (var m = 0; m <= 55; m+=5) {
                if (m==inst.selectedMinute){
            ...
            html +='</select>';
            if (showTimeAm==true){
                html +='<select onchange="DP_jQuery.datepicker._selectAm(\'#' + inst.id + '\',this)">';
                    if (inst.selectedHour>12){
                        html +='<option>am</option>';
                        html +='<option selected>pm</option>';
                    }else{
            ...
            html +='</div><button type="button" class="ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all" onclick="DP_jQuery.datepicker._hideDatepicker();">Ok</button>';
            html +='</div>';
        }
	...
	_formatDate: function(inst, day, month, year) {
		...
		var date = (day ? (typeof day == 'object' ? day :
			this._daylightSavingAdjust(new Date(year, month, day,inst.selectedHour,inst.selectedMinute,0))) :
			this._daylightSavingAdjust(new Date(inst.currentYear, inst.currentMonth, inst.currentDay,inst.selectedHour,inst.selectedMinute,0)));
...
$.widget("ui.tabs", {
    ...
    // Add create param to add tab
	add: function(url, label, index,create) {
