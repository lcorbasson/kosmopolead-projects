 ...
 */
/**
 * Modifications by AtlzA : http://www.atlza.com
 * DatePicker is freely distributable under the same terms as Prototype.
 * v0.9.2-1
 *
 */
...
DatePicker.prototype	= {
 Version	: '0.9.2',
 /*_typeshow : "linked" to set the datepicker linked to an input field, "fixed" to show it*/
 _typeshow : 'linked',
 _display : 'none',
 /*_relativeDiv : The div where the datePicker should appear if in "Fixed" mode */
 _relativeDiv	: null,
 /*_listImpDate : array of date the should appear whit the spacial class : "ImpDate", date format should be "dd-mm-yyyy" */
 _listImpDate : new Array(),
/*_onlyImpDateClick : define if only importante dates can be clicked or not : set to true for only importante dates */
 _onlyImpDateClick : false, 
 _relative	: null,
 _div		: null,
 _zindex	: 1,
 ...
 /*_selectDateCallback :function name to call if a date is clicked  */
 _selectDateCallback	: Prototype.emptyFunction,
 _date_separator: '/',
 _id_datepicker	: null,
 /* positionning */
 _topOffset	: 30,
 ...
 initialize	: function ( h_p ) {
  ...
  this._relative= h_p["relative"];
	if ( typeof(h_p["relativeDiv"]) != 'undefined' )
		this._relativeDiv= h_p["relativeDiv"];
	if ( typeof(h_p["listImpDate"]) != 'undefined' )
		this._listImpDate= h_p["listImpDate"];
	if ( typeof(h_p["onlyImpDateClick"]) != 'undefined' )
		this._onlyImpDateClick= h_p["onlyImpDateClick"];
	if ( typeof(h_p["typeshow"]) != 'undefined' )
		this._typeshow= h_p["typeshow"];
  if ( h_p["language"] )
  ...
  if ( typeof(h_p["clickCallback"]) == 'function' )
   this._clickCallback	= h_p["clickCallback"];
	/* Adds a callBack function to call when selected a date in fixed mode : must be a string */
	//alert(typeof(h_p["selectDateCallback"]));
  //if ( typeof(h_p["selectDateCallback"]) == 'function' )
  this._selectDateCallback = h_p["selectDateCallback"];	 
  ...
  this._div = Builder.node('div', { 
	...
	if( this._typeshow == "linked")
	{	
	  /* finally declare the event listener on input field if typeshow is "linked"*/
	  Event.observe(this._relative, 'click', this.click.bindAsEventListener(this), false);
	}
	else
	{
		Event.observe(window, 'load', this.click.bindAsEventListener(this), false);
	}
 },
 /**
 ...
 load		: function () {
	...
	{
		var todiv = document.getElementById(this._relativeDiv);
		todiv.appendChild( this._div );
	}
 ...
 _buildCalendar		: function () {
  ...
  for ( var i = 0; i < 6; i++ ) {
   ...
   for ( var j = 0; j < 7; j++ ) {
    ...
    if ( this._language == 'en' ) 
     var id	= $A([ this._leftpad_zero((h_ij["m"] +1), 2),
       this._leftpad_zero(h_ij["d"], 2), h_ij["y"] ]).join('-');
     else 
			var id_date = $A([ this._leftpad_zero(h_ij["d"], 2),
    ...
    /* set id and classname for cell if exists */
		...
		if ( this._listImpDate.in_array(id_date) )
		{
			td.className	= 'ImpDate';
		}
    else if ( h_ij["c"] )
     td.className	= h_ij["c"];
    /* on onclick : rebuild date value from id of current cell */
		if ( !this._onlyImpDateClick || this._listImpDate.in_array(id_date) )
		{
	    td.onclick	= function () { 
				if( _self._typeshow == "linked")
				{	  
				...
				{
					var selectedDate = String($(this).readAttribute('id')).replace(_self._relative+'-','').replace(/-/g,_self._date_separator); 			
					$(_self._relative).value = 	selectedDate ;
					eval(_self._selectDateCallback+'("'+ selectedDate +'")');
				}			
	    };
		}
    td.innerHTML= h_ij["d"];
