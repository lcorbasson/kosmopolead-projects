...
$.fn.extend({
	searchGrid : function ( p ) {
		...
		return this.each(function(){
			...
			modp = {top : p.top,left:p.left,width:p.width,height:p.height, modal:p.modal, drag:p.drag, resize:p.resize, destroy:true};
			if ( $("#"+IDs.themodal).html() != null ) {
				if( $.isFunction('beforeShowSearch') ) { p.beforeShowSearch($("#srchcnt"+gID)); }
				viewModal("#"+IDs.themodal,modp);
				...
				$("#sbut, #sreset").addClass('ui-state-default ui-corner-all').height(21)
					...
					if( $("#sval","#"+IDs.themodal).val() !="" ) {
						...
						if (es[0]===true) {
							...
							if(p.closeAfterSearch === true) {
								hideModal("#"+IDs.themodal,"#"+IDs.modalcontent);
							}
						} else {
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			modpe = {top : p.top,left:p.left,width:p.width,height:p.height, modal:p.modal, drag:p.drag, resize:p.resize, destroy:true};
			frmgr = "FrmGrid_"+gID,
			frmtb = "TblGrid_"+gID;
			if (rowid=="new") {
				rowid = "_empty";
			...
			if ( $("#"+IDs.themodal).html() != null ) {
				...
				$(frm).append(tbl);
				$(tbl).append("<tr id='FormError' class='ui-state-error' style='display:none'><td colspan='2'>"+"&nbsp;"+"</td></tr>");
				// set the id.
				...
				$(tbl).append("<tr id='Act_Buttons'><td class='navButton ui-widget-content'>"+bP+bN+"</td><td class='EditButton ui-widget-content'>"+bS+"&nbsp;"+bC+"</td></tr>");
				...
				viewModal("#"+IDs.themodal,modpe);
				if(onAfterShow) { onAfterShow($("#"+frmgr)); }
				$("#nData","#"+frmtb).addClass('ui-state-default ui-corner-all').append("<span class='ui-icon ui-icon-triangle-1-e'></span>");
				$("#pData","#"+frmtb).addClass('ui-state-default ui-corner-all').append("<span class='ui-icon ui-icon-triangle-1-w'></span>");
				$("#pData, #nData").hover(
				   function(){$(this).addClass('ui-state-hover');}, 
				   function(){$(this).removeClass('ui-state-hover');}
				);
								...
								complete:function(data,Status){
									...
									if(ret[0] === false) {
										...
										if(postdata.id=="_empty" ) {
											...
											if(rp_ge.closeAfterEdit) { hideModal("#"+IDs.themodal,"#"+IDs.modalcontent); }
										}
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true) {
						if(ind === false) {
							...
							if(nm == obj.p.ExpandColumn && obj.p.treeGrid === true) {
								...
								try {
									tmp =  $.unformat($("td:eq("+i+")",obj.rows[ind]),{colModel:obj.p.colModel[i]},i);
								} catch (_) {
									tmp = $.htmlDecode($("td:eq("+i+")",obj.rows[ind]).html());
								}
							}
						}
						...
						tdl = $("<td></td>").addClass("CaptionTD ui-widget-content");
						tde = $("<td></td>").addClass("DataTD ui-widget-content");
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			dtbl = "DelTbl_"+gID,
			modd = {top : p.top,left:p.left,width:p.width,height:p.height, modal:p.modal, drag:p.drag, resize:p.resize, destroy:true};
			if (isArray(rowids)) { rowids = rowids.join(); }
			if ( $("#"+IDs.themodal).html() != null ) {
				$("#DelData>td","#"+dtbl).text(rowids);
				$("#DelError","#"+dtbl).hide();
				if(onBeforeShow) { p.beforeShowForm($("#"+dtbl)); }
				viewModal("#"+IDs.themodal,modd);
						...
						if(!p.processing) {
							...
							$.ajax({
								...
								complete:function(data,Status){
									...
									//$(".loading","#"+IDs.themodal).fadeOut("fast");
									if(ret[0]) { hideModal("#"+IDs.themodal,"#"+IDs.modalcontent); }
								}
							});
						}
					}
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch) {
		...
		return this.each(function() {       
			...
			if ($("#"+alertIDs.themodal).html() == null) {
				...
				createModal(alertIDs,"<div>"+o.alerttext+"</div>",{drag:true,resize:true,caption:o.alertcap,top:vheight/2-25,left:vwidth/2-100,width:200,height:'auto'},$t.grid.hDiv,$t.grid.hDiv,true);
			}
			var tbd,
			navtbl = $("<table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table navtable' style='float:left'><tbody><tr></tr></tbody></table>"),
			sep = "<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>",
			pgid = $($t.p.pager).attr("id") || 'pager',
			tdwidth = $("tr td#"+pgid+"_"+o.position,"#"+pgid).width();
			if (o.add) {
				tbd = $("<td></td>");
				$(tbd).addClass('ui-pg-button ui-corner-all').append("<div class='ui-pg-div'><span class='ui-icon "+o.addicon+"'></span>"+o.addtext+"</div>");
				$("tr",navtbl).append(tbd);
				$(tbd,navtbl)
				.attr("title",o.addtitle || "")
				.click(function(){
			...
			if (o.edit) {
				...
				.click(function(){
					...
					if (sr) {
						...
						viewModal("#"+alertIDs.themodal,{modal:true,drag:true,resize:true,top:vheight/2-25,left:vwidth/2-100,width:200,height:'auto',destroy:false});
					}
					return false;
				})
				.hover( function () {
			...
			if (o.del) {
				...
				.click(function(){
					...
					else  { viewModal("#"+alertIDs.themodal,{modal:true, drag:true,resize:true,top:vheight/2-25,left:vwidth/2-100,width:200,height:'auto',destroy:false}); }
					return false;
				})
				.hover(
					function () {
			...
			//var test= $("<span style='visibility:hidden'></span>");
			//$(test).append(navtbl);
			//$("#gbox_"+$t.p.id).append(test);
			//alert($(navtbl).width());
			$("#"+pgid+"_"+o.position,"#"+pgid).append(navtbl);
			//.width(350);
			//$("tr td#"+pgid+"_"+o.position,"#"+pgid).css("width","352px");
			//$(test).remove();
		});
	},
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			if (findnav) {
				var tbd = $("<td></td>");
				$(tbd).addClass('ui-pg-button ui-corner-all').append("<div class='ui-pg-div'><span class='ui-icon "+p.buttonimg+"'></span>"+p.caption+"</div>");
